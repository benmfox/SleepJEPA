...
config_path = 'config/train_obj_sleepiness.yaml'
...
wsc_file_path = config['paths']['wsc_file_path']
mnc_file_path = config['paths']['mnc_file_path']
...
outcome_data = pd.read_csv(wsc_file_path)
mnc_data = pd.read_csv(mnc_file_path)

zarr_files = outcome_data.filepath.unique()
labels = outcome_data[outcome_cols[0]].values.tolist()
groups = [Path(i).stem.split('-')[-1] for i in zarr_files]
...
groups = [Path(i).stem.split('-')[-1] for i in train_zarrs]
...
if config['training']['use_class_weights']:
    class_weights = []
    train_outcome_data = outcome_data.loc[outcome_data['filepath'].isin(train_zarrs)]
    class0 = train_outcome_data[outcome_cols[0]].value_counts(dropna=False, normalize=False).sort_index().values[0]
    class1 = train_outcome_data[outcome_cols[0]].value_counts(dropna=False, normalize=False).sort_index().values[1]
...
name = config['run_config']['name']
filename = f"{name}-{outcome_cols[0]}" + "{epoch:02d}-AUROC-{val_auroc:.5f}-Loss-{val_loss:.5f}-AUPRC-{val_auprc:.5f}-all_score-{val_all_score:.5f}"
...
checkpoint_callback = ModelCheckpoint(dirpath=os.path.join(models_dir, sub_dir), save_top_k=1, monitor="val_auroc", mode='max', filename=filename)
checkpoint_callback2 = ModelCheckpoint(dirpath=os.path.join(models_dir, sub_dir), save_top_k=1, monitor="val_all_score", mode='max', filename=filename)
checkpoint_callback3 = ModelCheckpoint(dirpath=os.path.join(models_dir, sub_dir), save_top_k=1, monitor="val_auprc", mode='max', filename=filename)
...
callbacks = [checkpoint_callback, checkpoint_callback2, checkpoint_callback3]
...
if __name__ == "__main__":
    ...
    if Path(os.path.join(models_dir, f'{dataset_filename}-ext-test_samples.csv.gz')).exists():
        ...
        ext_test_zarrs = mnc_data.filepath.unique().tolist()
